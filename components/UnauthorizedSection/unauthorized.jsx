...
const {text,routes,buttons}=GLOBALLY_COMMON_TEXT
    ...
    <div className={`${Styles.wrapper}`}>
      <div className={`pb-[2%] max-md:flex-col ${Styles.errorcontainer}`}>
        ...
        <div className="flex flex-col p-[5%] justify-center">
          <h1 className="text-[35px]">{text.error403}</h1>
          <h1 className="text-[35px]">{text.unauthorisedUser}</h1>
          <hr className="bg-black h-[3px] w-[87.25%] mb-4" />
          <div className="w-[87.25%]">
            <p className={Styles.text}>{text.unauthorisedMessage}</p>
          </div>
          <div className="flex gap-4 mt-4 items-center">
            {userType === text.adminText ? (
              ...
              (userType === text.agentText || userType === text.individualText) && (
