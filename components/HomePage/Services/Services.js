...
const { everythingYouNeed } = HOME_PAGE_TEXT.searchSection
...
const Services = () => {
  ...
  useEffect(() => {
    const checkScreen = () => setIsMobile(window.innerWidth < 1145)
    checkScreen()
    ...
    <section className="w-[93%] lg:w-[93%] overflow-hidden border-none m-auto rounded-xl">
      <div className="bg-white rounded-xl">
        <div className="text-center w-full mb-4">
          <h2 className="inline-block text-primary mt-[2.5rem]">
            {everythingYouNeed.split(' ')[0]} {everythingYouNeed.split(' ')[1]}{' '}
            <h2 className="inline-block text-primary">
              {everythingYouNeed.split(' ')[2]}
            </h2>
          </h2>
        </div>
